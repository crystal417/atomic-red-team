<#
modified : crav3r@gmail.com

redcanary git hub 링크가 깨져서 변경 ("https://raw.githubusercontent.com/crav3r/atomic-red-team/round4test/atomics/T1059.001/src/")
ISE 요구 구문 제거
관리자 권한 요구 구문 제거 
- 관리자 권한은 캐시 삭제시 필요하다고 돼있으나, 관리자 권한을 가지더라도 오류 발생됨
- 캐시 삭제는 큰 의미 없음
일반 권한으로 실행해도 다운로드 동작에는 문제 없음
Hidden 방식으로 실행 제외 
sleep 기능을 Off
- 10초 sleep을 활성화하면 실행 중 스크립트가 끝까지 실해오디지 못하고  120초 경과되어 프로세스 종료되는 이슈있음
#>
...
ElseIf ($TLS -eq 1){
    ...
    $Url = @(        
        "https://raw.githubusercontent.com/crav3r/atomic-red-team/round4test/atomics/T1059.001/src/test.ps1",  # Basic Powershell Test script
        "test.dfir.com.au", # DNS text test - Powershell Test script base64 encoded in DNS txt field
        "https://raw.githubusercontent.com/crav3r/atomic-red-team/round4test/atomics/T1059.001/src/test.xml", # Powershell embedded command
        "https://raw.githubusercontent.com/crav3r/atomic-red-team/round4test/atomics/T1059.001/src/test.sct", # Powershell embedded scriptlet
        "https://raw.githubusercontent.com/crav3r/atomic-red-team/round4test/atomics/T1059.001/src/mshta.sct", # Powershell embedded scriptlet
        "https://raw.githubusercontent.com/crav3r/atomic-red-team/round4test/atomics/T1059.001/src/test.xsl" # Powershell embedded extensible Stylesheet Language
    )
}
